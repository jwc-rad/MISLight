...
def find_last_checkpoint(srcdir, query='', suffix='.ckpt'):
    files = find_files('', srcdir, suffix=suffix)
    files = [x for x in files if query in os.path.basename(x)]
    if len(files) > 0:
        fnumber = []
        for x in files:
            ns = re.findall(r'\d+', os.path.basename(x))
            if len(ns)>0:
                fnumber.append(int(ns[0]))
            else:
                fnumber.append(-100)
...
def WND(X, W, noclip=[False,False], preserve_range=False):
    clipmin = W[1] - 0.5*W[0]
    clipmax = W[1] + 0.5*W[0]
    ...
    if not preserve_range:
        R = (R.astype(np.float32) - clipmin)/W[0]
