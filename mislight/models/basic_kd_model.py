...
class BasicKDModel(SSLModel):
    '''Semi-supervised Learning - Basic Single Teacher & Student
    '''
    @staticmethod
    def get_model_specific_args():
        dummy = argparse.ArgumentParser()
        dummy = BasicKDModel.add_model_specific_args(dummy)
    ...
    def __init__(self, opt):
        ...
        if 'T' in self.model_names:
            self.netT = define_S(opt.nc_input, opt.num_classes, **netargs[1], opt=opt)
    ...
    def _step_S_labeled(self, stage):
        ...
        if bs > 0:
            ...
            lossT = self.criterionSeg(self.predT[self.labeled], self.label[self.labeled])
            loss_seg = lossS + lossT
            
            self.log(f'loss/{stage}_lossT_seg', lossT, batch_size=bs, on_step=(stage!='valid'), on_epoch=True)
        ...
        if bs > 0 and w0 > 0:
            ...
            loss_dis = self.criterionDis(self.predS[self.labeled]/temp_x, self.predT[self.labeled].detach()/temp_y) * comp
    ...
    def _step_S_unlabeled(self, stage):
        ...
        if bs > 0 and w0 > 0:
            ...
            if self.consistency_pseudosupervision:
                lossS = self.criterionCon(self.predS[~self.labeled]/temp_x, self.predT[~self.labeled].detach()/temp_y)
            else:
                lossS = 0
