...
class GenericAttentionUNetDecoder(nn.Module):
    def __init__(self, previous, num_classes, num_blocks_per_stage=None, props=None, transposed_conv=True, 
        ...
        for i, s in enumerate(np.arange(num_stages)[::-1]):
            ...
            self.attentions.append(AttentionBlock(spatial_dims, features_skip//2, features_skip, features_skip, self.props['dropout_op_kwargs']['p']))
    ...
    def forward(self, skips, return_layers=False):
        ...
        for i in range(len(self.tus)):
            x = self.tus[i](x)
            att = self.attentions[i](g=x, x=skips[i+1])
...
class GenericAttentionUNet(nn.Module):
